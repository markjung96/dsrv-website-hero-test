...
import OrbGradientAssetBackground from "../components/OrbGradientAssetBackground";
import VersionSelector from "../components/VersionSelector";

export default function OrbAssetPage() {
  return (
    <main className="relative h-screen overflow-hidden bg-black">
      <OrbGradientAssetBackground />

      <VersionSelector />

      <nav className="absolute top-0 left-0 right-0 z-20 flex items-center justify-center gap-1 px-4 py-4">
        <button
          className="px-4 py-2 text-[14px] font-semibold rounded-full transition-all duration-200 cursor-pointer bg-white/30 text-white backdrop-blur-md"
        >
          DSRV Portal
        </button>
      </nav>
