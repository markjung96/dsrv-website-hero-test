...
import SoftBlobBackground, { type SoftBlobVariant } from "../components/SoftBlobBackground";
import VersionSelector from "../components/VersionSelector";

const heroes: { variant: SoftBlobVariant; title: string; subtitle: string }[] = [
...
export default function SoftBlobRotatePage() {
  const [activeIdx, setActiveIdx] = useState(0);
  const { variant, title, subtitle } = heroes[activeIdx];

  return (
    <main className="relative h-screen overflow-hidden bg-white">
      <SoftBlobBackground key={variant} variant={variant} rotateGradient />
