...
// Flame-like blob: small detail, higher frequencies
float flameBlob(vec2 uv, float cx, float width, float dropBase, float t, float seed, float aspect) {
  float dx = (uv.x - cx) / width;
  float hFalloff = exp(-dx * dx * 1.8);
  float vy = 1.0 - uv.y;
  vec2 flameUV = vec2(uv.x * aspect, vy);
  float edge = dropBase
    + snoise(vec3(flameUV.x * 1.0, seed * 3.0 + vy * 0.3, t * 0.22)) * 0.16
    + snoise(vec3(flameUV.x * 2.0, seed * 7.0, t * 0.32 + vy * 0.5)) * 0.10
    + snoise(vec3(flameUV.x * 4.0, seed * 11.0, t * 0.50)) * 0.06
    + sin(flameUV.x * 2.5 + t * 0.40 + seed) * 0.04;
  float vMask = 1.0 - smoothstep(edge - 0.08, edge + 0.12, vy);
...
void main() {
  ...
  float blueness = max(b1, max(b2, max(b3, max(b4, max(b5, b6)))));
...
export default function FlameAuroraSmallBackground() {
