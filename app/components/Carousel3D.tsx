...
export default function Carousel3D({ items }: Carousel3DProps) {
  ...
  const getTextStyle = (slotIndex: number): React.CSSProperties => {
    const offset = slotIndex - position;
    const absOffset = Math.abs(offset);
    const translateX = offset * 110;
    const scale = 1 + absOffset * 0.05;
    ...
    return {
      ...
      opacity: Math.max(opacity, 0),
      zIndex: Math.round(20 - absOffset),
      pointerEvents: "none",
      transition: isDragging ? "none" : "opacity 0.3s ease",
    };
  };
  ...
  return (
    ...
    >
      <div className="relative" style={{ transformStyle: "preserve-3d", width: "clamp(250px, 26vw, 380px)", height: "clamp(340px, 36vw, 520px)" }}>
        {/* Card glass layer - 3D transforms */}
        <div className="absolute inset-0" style={{ transformStyle: "preserve-3d" }}>
          {slots.map((slotIndex) => {
            ...
            return (
              ...
              >
                <div
                  className="w-full h-full rounded-[40px] cursor-pointer overflow-hidden"
                  style={{
        ...
        {/* Text layer - flat, no 3D rotation */}
        <div className="absolute inset-0">
          {slots.map((slotIndex) => {
            ...
            return (
              <div
                key={slotIndex}
                className="absolute top-0 left-1/2 w-full h-full flex flex-col items-center justify-center px-8 text-center"
                style={getTextStyle(slotIndex)}
              >
                <div className="flex flex-col items-center gap-[16px] text-center">
                  <h3
                    className={`font-bold tracking-[-0.48px] text-center text-[clamp(32px,3.3vw,48px)] leading-[1.2] ${
                  ...
                  <p
                    className={`text-[clamp(14px,1.4vw,20px)] font-normal leading-[1.6] tracking-[-0.2px] text-center ${
                      isCardHovered ? "text-white" : "text-[#666]"
                    }`}
                  >
                    {item.subtitle}
                  </p>
